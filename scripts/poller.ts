 ...
 *
 * Calls the internal poll endpoint every 15 minutes.
 * Run with: npx ts-node scripts/poller.ts
 * Or via launchd: ~/Library/LaunchAgents/com.aditor.repurpose-poller.plist
 */
...
async function poll(): Promise<void> {
  ...
  try {
    ...
    const data = await res.json() as {
      processed: number;
      summary?: Array<{ rule_id: number; account: string; new_reels: number; posted: number; failed: number }>;
      elapsed_ms?: number;
      message?: string;
    };
